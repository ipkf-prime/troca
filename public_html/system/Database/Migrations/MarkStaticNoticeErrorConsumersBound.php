...
final class MarkStaticNoticeErrorConsumersBound extends Migration
{
    ...
    {
        $items = $this->items();
        if (count($items) !== 6) throw new RuntimeException('Notice/error binding count invalid.');

        $this->db->beginTransaction();

        try {
            foreach ($items as $item) $this->updateItem($item, $bound);
            $this->db->commit();
    ...
    private function updateItem(array $item, bool $bound): void
    {
        ...
        $definitionQuery = $this->db->prepare(
            'SELECT id, content_type, metadata_json FROM ui_content_definitions WHERE content_key = ? LIMIT 1 FOR UPDATE'
        );
        ...
        if ((string) ($definition['content_type'] ?? '') !== $type) throw new RuntimeException('Definition type mismatch: ' . $key);

        $metadata = $this->metadata($definition['metadata_json'] ?? null);
        $this->assertOwnership($key, $mode, $metadata);
        $metadata['consumer_bound'] = $bound;

        $updateDefinition = $this->db->prepare('UPDATE ui_content_definitions SET metadata_json = ? WHERE id = ?');
    ...
    private function items(): array
    {
        ...
        $items = array_values(array_filter(
            ...
            static fn (mixed $item): bool =>
                is_array($item)
                && in_array(($item['content_type'] ?? null), ['notice', 'error'], true)
        ));

        foreach ($items as $item) {
            if (($item['consumer_bound'] ?? false) !== true) {
                throw new RuntimeException('Catalog item not bound: ' . ($item['key'] ?? ''));
            }
        }
